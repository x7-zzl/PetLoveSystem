...
//    内连接查询
            String sql = "select m.masterId,m.masterName,p.petId,p.petName,p.petAge,p.petSex,p.petType from masterpet m inner join petinfo p on m.petId=p.petId where p.petId=? order by p.petId";
...
//        将游标移动到最后一行
        ...
        <tr>
            <td colspan="1" align="center">
                <a href="petImgShow.jsp?petId=<%=rs.getString("petId")%>&petName=<%=rs.getString("petName")%>">
                    <%=rs.getString("petId")%>
                </a>
            </td>
            <td colspan="1" align="center">
    ...
    <div class="a">
        ...
        <%--        不能有空格，否则会被读入--%>
        <a target="mainFrame" class="a" href="petVisPetInfo.jsp?page=<%=intPage+1%>">下一页</a>
        <%
            }
            if (intPage > 1) {
        %>
        &nbsp;
        <a target="mainFrame" class="a" href="petVisPetInfo.jsp?page=<%=intPage-1%>">上一页</a>
        <%
            }
            try {
                rs.close();
    ...
    <a href="petVisit.jsp"><img src="image/return_login_1.png" alt="返回"></a>
</div>
</body>
</html>
